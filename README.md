...
Перед запуском CLI спрашивает, **сколько спикеров в записи** (только AssemblyAI). Первый пункт — «Не знаю / неважно», он выбирается одним Enter: тогда число определяется автоматически. Точное число — подсказка диаризации, она заметно повышает качество разделения.
...
Если спикер один (лекция, стрим, монолог), метка спикера не печатается. Реплики склеиваются в блоки не длиннее минуты — иначе весь транскрипт стал бы одним блоком с единственным таймстампом:
...
CLI понимает обе схемы хранения Meet: новую **Google Meet** (с июля 2026 — подпапка на каждую встречу, старая папка переезжает внутрь как «Legacy Meet Recordings») и старую плоскую **Meet Recordings**. Ярлыки на записи (они лежат в «Google Meet» у участников встречи) тоже подхватываются, если у SA есть доступ к самому файлу записи. Если файл записи назван кодом встречи, а осмысленное название — на папке встречи, имя транскрипта берётся с папки.
...
Если на SA расшарены папки нескольких Google-аккаунтов, перед записью стоит цветная метка аккаунта-владельца (`[londeren]`, для доменных аккаунтов — полный адрес; у записей без владельца, например на общем диске, — `[?]`), а над списком — сводка «Аккаунты: londeren ×200, grow ×47». Метка участвует в поиске: `londeren 09` покажет сентябрьские записи этого аккаунта.
...
8. Откройте **Google Drive** → папку **Google Meet** → ПКМ → **Share** → вставьте email SA → **Editor** → **Send**
   - Если у вас ещё есть старая папка **Meet Recordings** со старыми записями и она лежит не внутри «Google Meet» — расшарьте и её
...
3. Папка Google Meet (и старая Meet Recordings, если есть) должна быть расшарена на SA как **«Редактор»** (не «Просмотр») — иначе Google вернёт ошибку прав.
...
Число спикеров CLI спрашивает **до** запуска — `speakers_expected` принимается только при отправке задачи, а уточнение постфактум означало бы вторую платную транскрипцию. Не знаете — выбирайте «Не знаю / неважно». У Deepgram такой ручки нет (число определяется автоматически).

Если диаризация не сработала (AssemblyAI вернул одного спикера и одну реплику на всю запись — бывает на длинных стримах), CLI сам забирает у того же уже оплаченного задания разбивку по абзацам и ставит таймстампы по ним, предупредив в консоли.
