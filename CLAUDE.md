...
- [gdrive.js](gdrive.js) — Google Drive API и Service Account. Тоже автономен: умеет искать Meet Recordings, скачивать файл, импортировать SA-ключ, чистить авто-имена Meet (`cleanMeetName`: срезает «– Recording»/таймзоны, нормализует дату, ловит дефолтные коды встреч `xxx-xxxx-xxx`). `getDriveClient(write)` запрашивает scope `drive.readonly` по умолчанию и полный `drive` только при `write=true` (least privilege); `renameDriveFile(drive, id, name)` — metadata-only переименование исходника на Диске (нужен Editor-доступ SA, иначе 403).
...
- `config.json` — API-ключ Deepgram, опции транскрипции (`autoLang`, `lang`, `speakers`, `mergeUtterances`, `numerals`), папки, имена спикеров. Опции редактируются единым чеклистом (`@inquirer/checkbox`) в Настройках (`editTranscriptionOptions` в [app.js](app.js)); **перед запуском не спрашиваются** — потоки получают их из конфига через чистый `optionsFromConfig` (отсутствие ключа = вкл) и печатают dim-сводку `Опции: ...`. Ярлык на рабочий стол предлагается только в Настройках (вопроса на первом запуске больше нет).
- `service-account.json` — SA-ключ Google (не смешиваем с config)

Опция `renameDriveSource` (по умолчанию `false`) — после транскрипции переименовывает исходную запись Meet на Диске в имя транскрипта. Срабатывает только для generic-записей (`isGeneric`), спрашивает подтверждение, ловит ошибку прав. При включении `getMeetRecordings({ write: true })` берёт полный scope `drive`; SA при этом должен иметь доступ «Редактор» к папке Meet Recordings, иначе `files.update` вернёт 403 (транскрипт всё равно сохранится).
