...
- [gdrive.js](gdrive.js) — Google Drive API и Service Account. Тоже автономен: ищет записи Meet в обеих схемах хранения (см. «Google Meet flow»: корни `MEET_ROOT_NAMES` → подпапки → один глобальный запрос медиа), скачивать файл, импортировать SA-ключ, чистить авто-имена Meet (`cleanMeetName`: срезает «– Recording»/таймзоны, нормализует дату, ловит дефолтные коды встреч `xxx-xxxx-xxx`; `recordingName(file)` поверх него берёт имя с папки встречи, если файл назван кодом). `getDriveClient(write)` запрашивает scope `drive.readonly` по умолчанию и полный `drive` только при `write=true` (least privilege); `renameDriveFile(drive, id, name)` — metadata-only переименование исходника на Диске (нужен Editor-доступ SA, иначе 403).
...
- [assembly.js](assembly.js) — облачный провайдер транскрипции **AssemblyAI** (`transcribeAssembly`): транскрипт + диаризация в одном вызове, с подсказкой числа спикеров `speakers_expected`. Загрузка → запуск → поллинг; возвращает `utterances` в формате Deepgram (`{speaker, start, end, transcript}`), так что `formatMarkdown`/переименование не меняются. **Схлопнувшаяся диаризация:** на длинных записях AssemblyAI иногда отдаёт одного спикера и одну utterance на весь файл — транскрипт вырождается в стену текста с единственным таймстампом. `isDiarizationCollapsed` ловит это: один спикер И реплики реже одной в минуту (`COLLAPSE_DENSITY_S = 60`) при длительности от 2 минут (`COLLAPSE_MIN_DURATION_S = 120`) — оба порога binding, на них есть граничные тесты. Дальше реплики берутся из `GET /v2/transcript/{id}/paragraphs` — это тот же уже оплаченный job, повторно не биллится (`calls.submit === 1` проверяется тестом). Подменяем, только если абзацев строго больше И они покрывают ≥90% текста (`COVERAGE_MIN_RATIO`): частичный ответ иначе молча срезал бы часть оплаченного транскрипта. Флаг `diarizationFallback` в ответе `transcribeAssembly`/`transcribeToUtterances`/`transcribeFile` → CLI предупреждает пользователя. Сетевые ошибки и не-2xx гасятся (транскрипт уже получен), ошибка в самом коде фоллбэка логируется со стеком. Альтернатива Deepgram, когда нужно задать число спикеров (у Deepgram такой ручки нет).
...
4. Транскрипция по `provider`. **AssemblyAI** (`assembly`) — `transcribeAssembly`: транскрипт + диаризация. Число спикеров спрашивается **до** запуска (`askSpeakersExpected` → `opts.numSpeakers` → `speakers_expected`): ручка принимается только при сабмите, поэтому уточнять постфактум нельзя — это второй платный job. В быстром режиме — флаг `-n/--speakers-expected`. **Deepgram** (дефолт) — весь файл загружается в память (`readFileSync`) и одним POST уходит в Deepgram. Параметры собираются в [callDeepgram](transcribe.js#L156): всегда `smart_format/punctuate/paragraphs/utterances`; язык — либо `detect_language=true` (если `autoLang`), либо `language=<код>` (взаимоисключающие); спикеры — `diarize_model=latest` (**новый v2-диаризатор; legacy `diarize=true` слать НЕЛЬЗЯ — Deepgram отклонит запрос при обоих**); `numerals=true` опционально. Audio-intelligence фичи (`summarize`/`topics`/`sentiment`) — **только английский**, для русского не подключать (`summarize`+`language=ru` = 400, роняет весь запрос).
5. Результат форматируется в Markdown. Два режима вывода:
   - `speakers=true` → блоки `**Name** [ts]` по `results.utterances`. При `merge=true` (дефолт) подряд идущие реплики одного спикера склеиваются в один блок (таймстамп — от первой); при `merge=false` — по одной. **Склейка ограничена по времени:** блок закрывается, когда его длительность доходит до `MERGE_MAX_SPAN_S = 60` ([engine.js](engine.js)). Внутри диалога реплики короче, поэтому поведение не меняется; у монолога (один спикер на всю запись) без этого потолка склейка сливала ВСЁ в один блок и от транскрипта оставался единственный таймстамп. **Метка спикера** не печатается, если спикер один и имени для него не задано — различать нечего, формат блока становится `**[ts]**` + текст. Это breaking change для внешних потребителей `engine.js`, которые ищут `**Speaker A**` регуляркой по markdown (переименование через `speakerNames` не затронуто — с именем метка возвращается).
...
**Где лежат записи на Диске — две схемы.** До июля 2026 Meet складывал всё плоско в `My Drive/Meet Recordings/`. С июля 2026 (анонс Workspace Updates 22.07.2026) — в `My Drive/Google Meet/<название или код встречи> - YYYY/MM/DD HH:MM TZ/` (подпапка на встречу, recurring делят одну; рядом с mp4 лежат Chat/транскрипт/заметки Gemini), а старую папку Google переносит внутрь как «Legacy Meet Recordings». Спец-маркеров у папок нет — только имена (`MEET_ROOT_NAMES`). `collectRecordings` работает так: корни по точному имени (+ `contains 'Meet Recordings'` для переименованных руками) → их подпапки на один уровень → **один** глобальный запрос медиа, доступных SA, с локальным фильтром по родителю (не по запросу на папку — папок встреч со временем сотни; `paginateFiles` листает дальше, пока не наберёт `limit` отфильтрованных). Ярлыки (`shortcut`) на медиа — у участников встречи в их «Google Meet» лежат именно они — разворачиваются в целевой файл (`id`/`mimeType` подменяются, исходный в `shortcutId`), при дубле с настоящим файлом остаётся файл; у остальных ярлыков `files.get` дочитывает имя/размер/дату цели (у ярлыка `size` пустой, а `createdTime` — его собственный), и если цель SA недоступна (файл организатора не расшарен на SA) — запись выпадает из списка, потому что скачать её всё равно нельзя. Записи из папки встречи получают `folderName`; `recordingName` берёт оттуда имя, если файл назван кодом. Возвращает `{files, roots}`; `roots` идёт в UI как сводка «папки: Google Meet ×1, Meet Recordings ×2» и подсказка «расшарьте Google Meet», если такого корня нет — иначе список молча протухает. Если корней нет вовсе — все медиа SA без фильтра. У каждой записи есть `account` (`accountLabel` от `owners[0].emailAddress`: gmail → имя ящика, домен → весь адрес, Shared Drive → пусто); UI показывает его цветной меткой перед именем и включает в строку поиска, но только когда аккаунтов больше одного.

`collectRecordings(drive, limit, opts)` используется и снаружи — оркестратором Meet-записей (`orkestrator-gg`, OAuth под самим аккаунтом, не SA). Для него две опции: `ownedOnly` (в запросы корней и медиа добавляется `'me' in owners`, ярлыки не берутся — иначе OAuth-клиент подхватит чужие Meet-папки, расшаренные аккаунту; без найденных корней результат пустой, а не «все видео аккаунта», как в SA-fallback) и `rootIds` (явные корни по ID из конфига в дополнение к найденным по имени; недоступный ID — ошибка, а не пустой список). Экспорты gdrive.js — публичный API пакета: удаление/переименование экспорта ломает оркестратор на старте, поэтому это major-изменение или как минимум проверка `grep` по orkestrator-gg.
