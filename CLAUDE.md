...
3. Если расширение не в `DIRECT_AUDIO` (whitelist: `.wav .mp3 .ogg .flac .m4a .opus .webm`) — `ffmpeg` конвертирует в opus (`convertToOpus`/`opusEncodeArgs`). Два режима по `hq` (= `speakers`):
   ...
   - **hq (диаризация)** — 128 kbps + сохраняются каналы (не mono, ~56 MB/ч). Диаризация v2 строит голосовые эмбеддинги по тонким спектральным признакам; на 32k mono у тихих/редких спикеров они теряются и Deepgram их сливает (реальный баг на 5-спикерном созвоне: 32k→3/5, 96k→4/5, 128k — потолок из ресёрча). Подсказки числа спикеров у Deepgram нет — качество аудио единственный рычаг. Для гарантированного N — локальный pyannote (`num_speakers`), см. `local-engine`.
